...
for ((i=0; i<number; i++)) do
    ...
    te[i]=$(cat ${file[i]} | awk -v th=$threshold '$3+0>th{print $14;exit}')
    tv[i]=$(cat ${file[i]} | awk -v th=$threshold '$3+0>th{print $15;exit}')
    tg[i]=$(cat ${file[i]} | awk -v th=$threshold '$3+0>th{print $16;exit}')
    
    p[i]=$(echo ${p[i]} | sed 's/^0*//')
    p[i]=$(echo ${p[i]} | sed 's/,//')
    te[i]=$(echo ${te[i]} | sed 's/^0*//')
    te[i]=$(echo ${te[i]} | sed 's/,//')
    tv[i]=$(echo ${tv[i]} | sed 's/^0*//')
    tv[i]=$(echo ${tv[i]} | sed 's/,//')
    tg[i]=$(echo ${tg[i]} | sed 's/^0*//')
    tg[i]=$(echo ${tg[i]} | sed 's/,//')
    printf "%d %d %e %e %e \n" ${point[i]} ${p[i]} ${te[i]} ${tv[i]} ${tg[i]}
